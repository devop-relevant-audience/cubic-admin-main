...
const Navbar = ({}) => {
  ...
  return (
    <div className=''>
      <div className={cn(' fixed z-10 w-full h-20 pl-[55px] pr-[64px] flex justify-between items-center  bg-[#282828]  ', pathname === '/' ? 'hidden' : '')}>
        <div className='cursor-pointer' onClick={() => setShowSidebar(!showSidebar)}>
          <HamburgerIcon />
        </div>
        <div>
          <Image src='/logo.avif' alt='gymx logo' width={200} height={200} />
        </div>
