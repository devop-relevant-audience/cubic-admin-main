...
const CubicTagDialog = ({ isTrainerTag = false }) => {
  ...
  useEffect(() => {
    const loadTag = async () => {
      const { data } = await cubicApi.get(
        `/${isTrainerTag ? "trainertag" : "tag"}`
      );
      setTag(isTrainerTag ? data.trainertags : data.tags);
    };
    loadTag();
  }, [trigger]);
  ...
  const addTag = async () => {
    ...
    const { data } = await cubicApi.post(
      `/${isTrainerTag ? "trainertag" : "tag"}`,
      {
        tag: newTag,
      }
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      onClick={async () => {
                        await cubicApi.delete(
                          `/${isTrainerTag ? "trainertag" : "tag"}/${item._id}`
                        );
                        setTrigger(trigger + 1);
                      }}
                    >
