...
export const excelColumnIndex = generateExcelColumnIndex().reduce(
  (array, item) =>
    array.concat(generateExcelColumnIndex().map((v) => `${item}${v}`)),
...
export class ExcelExport {
  ...
  resolveCellFormula(cell, formula) {
    ...
    const callbackFormula = (tableSettings) => {
      ...
      formula = formulaKeys.reduce((result, formulaKey) => {
        const [tableName, tableColumnKey, cellIndex] = formulaKey
          .slice(1, -1)
          .split(",");
        const tableContentIndex =
          this._tableContentIndex[
            Object.keys(tableSettings).findIndex((v) => v === tableName)
          ];
        let columnIndex;
        ...
        if (isNaN(tableColumnKey)) {
          const index = tableSettings[tableName].headerDefinition.findIndex(
            (v) => v.key === tableColumnKey
          );
        ...
        const rowIndex =
          cellIndex > 0
            ? tableContentIndex.start + Number(cellIndex) - 1
            : tableContentIndex.end + Number(cellIndex);
  ...
  getTableHeaders({ headerDefinition, headerGroups = [] }) {
    ...
    let columns = headerDefinition.map((column) =>
      headerGroups.find((group) => group.key === column.groupKey)
    );
    ...
    while (columns.filter((v) => !!v).length > 0) {
      ...
      columns = columns.map(
        (column) =>
          column && headerGroups.find((group) => group.key === column.groupKey)
      );
    }
    return headers;
  }
  ...
  addVerticalBorderLineForTables(data, headerHeight, { headerDefinition }) {
    for (
      let i = this._activedRowIndex - headerHeight;
      i < this._activedRowIndex + data.length;
      i++
    ) {
   ...
   */
  addHorizontalBorderLineForTableContent(
    row,
    rowValues,
    tableContent,
    tableContentIndex
  ...
  addFormula(data, { headerDefinition }) {
    ...
    headerDefinition.forEach((column, index) => {
        ...
        for (let level = 0; level < data.length; level++) {
          data.forEach((v, rowIndex) => {
            if (v.level !== undefined && v.level === level) {
              const rowNums = data
                .map((t, i) => (t.parentId === v.id ? i : undefined))
              ...
              if (formula) {
                const cell = this._ws.getCell(
                  `${columnIndex}${this._activedRowIndex + rowIndex}`
                );
      ...
      if (column.rowFormula) {
        const { formulaKeys, columnKeys } = this.resolveRowFormula(
          column.rowFormula
        );
        data.forEach((v, index) => {
          const currentRowIndex = this._activedRowIndex + index;
          const leafDataNorNot = hierarchyColumn
            ? data.findIndex((t) => t.parentId === v.id) === -1
            : true;
          // Leaf node or cell with percentage data-type need to add row level formula
          if (
            column.dataType === defaultDataType.percentage ||
            leafDataNorNot
          ) {
            const formula = formulaKeys.reduce((result, formulaKey, index) => {
              const columnKeyIndex = headerDefinition.findIndex(
                (column) => column.key === columnKeys[index]
              );
              return result.replace(
                new RegExp(formulaKey, "g"),
  ...
  addTableColumns(headerDefinition) {
    ...
    this._ws.columns = headerDefinition.map((column) => {
      // If data type is String, set cell to middle left alignment
      if (!column.dataType) {
        column.style = { ...column.style, alignment: alignment.middleLeft };
      }
      // If data type is Number and Percentage, set number format
      if (
        [defaultDataType.number, defaultDataType.percentage].includes(
          column.dataType
        )
      ) {
        column.style = {
  ...
  addTableHeaders({ headerDefinition, headerGroups }) {
    ...
    headers.forEach((header, index) => {
      ...
      // Set row height based on header group
      const group = headerGroups[index];
      if (group && group.height) {
        row.height = group.height;
      }
    });

    headers.forEach((header, index) =>
      this.mergeCellForHeader(
        header.map((v) => (v && v.key) || null),
        this._activedRowIndex + index
      )
    );
    ...
    data.forEach((value, index) => {
      ...
      if (cellsHasStyleSetting.length > 0) {
        row.eachCell({ includeEmpty: true }, (cell, cellNumber) => {
          ...
          if (
            cellsHasStyleSetting.includes(cellNumber) &&
            objectData !== null &&
  ...
  setGapBetweenTwoTables(gapBetweenTwoTables) {
    this._activedRowIndex =
      this._activedRowIndex + gapBetweenTwoTables || defaultGapBetweenTwoTables;
  }
  ...
  createWorkSheet(data, wsSetting, wsIndex) {
    ...
    Object.keys(tableSettings).forEach((index) => {
      ...
      if (tableSetting.headerDefinition) {
        ...
        this.addVerticalBorderLineForTables(
          tableData,
          headerHeight,
          tableSetting
        );
    ...
    this._callCellFormula.forEach((callbackFunc) =>
      callbackFunc(tableSettings)
    );
  }
...
// const ExcelExport = dynamic(async () => await import("export-xlsx"), {
//   ssr: false,
// });
...
const useExportCSV = ({
  ...
  const exportExcel = (data = []) => {
    const { SETTINGS_FOR_EXPORT, dataExport } = generateExcel(data);
          ...
          tableSettings: {
            data: {
              ...
              headerGroups: headerGroups.map((group) => ({
                ...group,
                height: group.height || 20, // Default height if not specified
              })),
              headerDefinition,
            },
          },
